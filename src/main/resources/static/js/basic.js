...
function showProduct(folderId = null) {
    ...
    $('#pagination').pagination({
        ...
        locator: 'content',
        alias: {
            pageNumber: 'page', //조회할 페이지 번호 (1부터 시작)
            pageSize: 'size' //한 페이지에 보여줄 상품 개수 (10개로 고정!)
        },
        totalNumberLocator: (response) => {
            return response.totalElements;
        },
        pageSize: 10, //한 페이지당 표시할 아이템 수
        showPrevious: true, //이전 페이지 버튼을 표시할지 여부를 설정합니다.
        showNext: true, //다음 페이지 버튼을 표시할지 여부를 설정합니다.
        ajax: {
            beforeSend: function () {
...
function addFolder() {
    ...
    $.ajax({
        ...
        .fail(function(xhr, textStatus, errorThrown) {
            alert(xhr.responseJSON.errorMessage); //api 예외처리 적용
